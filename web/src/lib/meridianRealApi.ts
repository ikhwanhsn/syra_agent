...
const real = () => `${getApiBaseUrl().replace(/\/$/, "")}/experiment/meridian-real`;
...
export type MeridianEngineHealth = {
  running: boolean;
  pid: number | null;
...
export type MeridianRealState = {
  ...
  engine?: MeridianEngineHealth;
};

export async function fetchMeridianRealState() {
  return parseJson<MeridianRealState>(
    await fetch(`${real()}/state`, { credentials: "include" }),
  );
}

export async function fetchMeridianEngineHealth() {
  return parseJson<MeridianEngineHealth>(
    await fetch(`${real()}/engine/health`, { credentials: "include" }),
  );
}

export async function fetchMeridianRealPositions(
...
export async function disableMeridianReal(opts: { closeAll?: boolean } = {}) {
  return parseJson<MeridianRealState>(
