...
const STORAGE_KEY = "syra-post-x-status";
...
export function setPostXStatus(updateNumber: number, posted: boolean): void {
  ...
  writeStatusMap(map);
  window.dispatchEvent(new CustomEvent("syra-post-x-status-change"));
}
...
export function usePostXStatus(updateNumber: number, defaultPosted = false) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("syra-post-x-status-change", onCustom);
    return () => {
      window.removeEventListener("storage", onStorage);
      window.removeEventListener("syra-post-x-status-change", onCustom);
    };
  }, [refresh]);
