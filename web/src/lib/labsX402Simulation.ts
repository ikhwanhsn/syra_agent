...
export const AUTO_BATCH_INTERVAL_MIN_MINUTES = 15;
export const AUTO_BATCH_INTERVAL_MAX_MINUTES = 60;
...
export interface LabsX402SimulationInput {
  payerCount: number;
  /** Ignored: scheduler uses a random 15-60 min batch wait. */
  intervalMin?: number;
  ...
  if (payerCount <= 0) {
    ...
    if (grossUsdcPerDay + 0.005 < targetVolumeUsd) {
      ...
      if (neededPayers > payerCount) {
        achievementHints.push(
          `Add ~${neededPayers - payerCount} more payer wallet${neededPayers - payerCount === 1 ? "" : "s"} (≈${neededPayers} total) at the random 15-60 min batch wait` +
            (priceMultiplier < 100
        ...
        achievementHints.push(
          `Aim for ~${targetCallsPerDay.toLocaleString()} calls/day (~${formatUsd(projectedTargetGrossUsd)} at avg effective price). Batch wait is random 15-60 min.` +
            (priceMultiplier < 100
