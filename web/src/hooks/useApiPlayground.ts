...
function getNansenBaseUrl(): string {
  const base = import.meta.env.VITE_NANSEN_API_BASE_URL;
...
export function useApiPlayground() {
  ...
  const sendRequest = useCallback(
    async (
      ...
      const payerAddress = walletContext.address ?? null;
      if (payerAddress) {
  ...
  const pay = useCallback(async () => {
    if (!paymentOption) {
      ...
      toast({
        ...
        description: `This endpoint requires a payment method Syra does not support yet. Only Solana x402 payments are available.`,
        variant: "destructive",
      });
      return;
    }
