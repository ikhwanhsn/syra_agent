...
async function persistAvatarIfMissing(
  ...
  const stored = typeof fromApi === "string" ? fromApi.trim() : "";
  if (stored || !resolved) return;
  try {
    await agentWalletApi.updateAvatar(anonymousId, resolved);
  } catch {
    /* guest still sees deterministic avataaars URL */
  }
}

function syncAvatarFromApi(
  anonymousId: string | null | undefined,
  fromApi: string | null | undefined,
  setAvatarUrl: (v: string | null) => void,
): string | null {
  const resolved = resolveUserAvatarUrl(fromApi, anonymousId ?? undefined);
  setAvatarUrl(resolved);
  const id = typeof anonymousId === "string" ? anonymousId.trim() : "";
  if (id && resolved) void persistAvatarIfMissing(id, fromApi, resolved);
  return resolved;
}
...
function restoreWalletCacheToState(
  ...
  setters.setAvatarUrl(resolveUserAvatarUrl(cached.avatarUrl, cached.anonymousId));
    ...
    const syncWallet = async () => {
      try {
        ...
        if (cachedFallback) {
          ...
          setAvatarUrl(
            resolveUserAvatarUrl(cachedFallback.avatarUrl, cachedFallback.anonymousId),
          );
      ...
      agentWalletApi
        ...
        .catch(() => {
          ...
          if (cached) {
            ...
            setAvatarUrl(resolveUserAvatarUrl(cached.avatarUrl, id));
