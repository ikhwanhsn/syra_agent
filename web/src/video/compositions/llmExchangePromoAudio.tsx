/**
 * Declarative SFX + BGM for LLM Exchange promo.
 * from frames are relative to scene starts via PROMO_SCENES.
 */
...
export type LlmExchangePromoAudioProps = {
  bgm?: boolean;
  durationInFrames?: number;
};

export function LlmExchangePromoAudio({
  bgm = true,
  durationInFrames = LLM_EXCHANGE_PROMO_DURATION,
}: LlmExchangePromoAudioProps) {
  const fadeIn = 18;
  const fadeOut = 40;
  ...
  return (
    <>
      {bgm ? (
        <Audio src={BGM} volume={bgmVolume} loop />
      ) : null}
      {CUES.map((cue, i) => (
        <Sequence
          key={`${cue.note}-${i}`}
