...
export function ShareChatModal({
  ...
  const handleSaveSetting = useCallback(() => {
    toast({
      ...
      description: isSharePublic ? "Chat is now public." : "Chat is now private.",
    });
  }, [isSharePublic, toast]);
  ...
  return (
    <Modal
      ...
      description="Choose who can view this chat. New shares are private by default."
      footer={
        <>
    ...
    >
      <div className="space-y-4">
        <div>
          <p className="mb-2 text-sm font-medium text-foreground">Link to share</p>
          <div className="flex gap-2">
            <Input
              readOnly
              value={shareLink}
              className="min-w-0 flex-1 border-border bg-muted/50 font-mono text-xs"
            />
            <Button
              variant="secondary"
              size="icon"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              >
                <Lock
                  className={cn(
                    "h-5 w-5",
                    !isSharePublic ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground",
                  )}
                />
              </div>
              <span
                className={cn(
                  "font-medium",
                  !isSharePublic ? "text-amber-700 dark:text-amber-300" : "text-foreground",
                )}
              >
                Private
              </span>
            ...
            >
              ...
              <span
                className={cn(
                  "font-medium",
                  isSharePublic ? "text-emerald-700 dark:text-emerald-300" : "text-foreground",
                )}
              >
                Public
              </span>
