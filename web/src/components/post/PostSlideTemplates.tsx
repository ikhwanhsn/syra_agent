      ...
      return (
        ...
        >
          ...
          <PostReveal isActive={isActive} delayMs={200}>
            <p className="post-slide-copy post-slide-prose post-slide-balance text-center">
              {slide.body}
            </p>
          </PostReveal>
        </PostSlideLayout>
        ...
        >
          <PostReveal isActive={isActive} delayMs={0}>
            <div className="post-accent-panel">
              ...
              <h2 className="post-slide-headline mt-2 text-left">
                {slide.headline}
              </h2>
              <p className="post-slide-copy post-slide-prose mt-3 text-left">
                {slide.body}
              </p>
            </div>
          </PostReveal>
    ...
    case "statement-large-type":
      return (
        ...
        >
          ...
          <PostReveal isActive={isActive} delayMs={100}>
            <h2 className="post-slide-headline post-slide-headline--display post-slide-balance text-center">
              {slide.headline}
            </h2>
          </PostReveal>
          <PostReveal isActive={isActive} delayMs={220}>
            ...
            {slide.highlights.map((item, i) => (
              <PostReveal key={item} isActive={isActive} delayMs={280 + i * 90}>
                <li className="post-highlight-row flex gap-2.5 rounded-lg border border-white/[0.07] bg-gradient-to-r from-white/[0.06] to-transparent px-3 py-2.5">
                  <span className="post-check shrink-0 font-mono text-[#F3BA2F]/80">
                    ✓
                  </span>
    ...
    case "hero-split":
      return (
        <PostSlideLayout
          ...
          variant="grid"
          itemCount={slide.highlights.length}
          className="post-tmpl-hero-split"
        >
            ...
            {slide.highlights.map((item, i) => (
              <PostReveal key={item} isActive={isActive} delayMs={280 + i * 80}>
                <li className="post-slide-card post-highlight-row flex gap-2 px-3 py-2">
                  <span className="font-mono text-[10px] text-[#F3BA2F]/75">
            ...
            {slide.highlights.slice(0, 3).map((item, i) => (
              <PostReveal key={item} isActive={isActive} delayMs={200 + i * 90}>
                <div className="post-slide-card post-pillar text-center">
                  <p className="font-mono text-xs text-[#F3BA2F]/70">
                    {String(i + 1).padStart(2, "0")}
                  </p>
              ...
              >
                <div className="post-flow-card post-slide-card group relative h-full">
                  {index < slide.steps.length - 1 ? (
                    <span
                      className="post-flow-connector absolute -right-1 top-1/2 hidden h-px w-2 -translate-y-1/2 sm:block"
                      aria-hidden
                    />
                  ) : null}
                  <p className="post-slide-meta text-[#F3BA2F]/70">
                    {step.step}
                  </p>
              ...
              >
                <li className="post-numbered-flow-item">
                  ...
                  <div className="min-w-0">
                    ...
                    <span className="post-slide-card-copy ml-2">
                      {step.description}
                    </span>
                  </div>
                </li>
    ...
    case "cards-row":
      return (
        ...
        >
          ...
          />
          <div
            className="post-slide-grid post-slide-grid--row"
            data-cols={slide.cards.length}
        ...
        >
          ...
          <PostSlideGrid count={slide.items.length}>
            {slide.items.map((item, index) => {
              const cardClassName =
                "post-surface-card post-slide-card group relative flex h-full min-h-0 flex-col";
              const inner = (
                <>
            ...
            {slide.items.map((item, index) => (
              ...
              >
                <li className="post-surface-list-item">
                  <item.icon
                    className="h-4 w-4 shrink-0 text-[#F3BA2F]/65"
                    strokeWidth={1.5}
                  />
    ...
    case "impact-featured-stat":
      return (
        ...
        >
          ...
          <PostSlideGrid
            count={Math.max(0, slide.stats.length - 1)}
    ...
    case "closing-links":
      return (
        ...
        >
          ...
          <PostReveal isActive={isActive} delayMs={120}>
            <p className="post-slide-copy post-slide-prose post-slide-balance">
              {slide.subline}
            </p>
          </PostReveal>
          <PostSlideGrid
            count={slide.links.length}
            className="post-slide-link-grid"
          >
            {slide.links.map((link, index) => (
              <PostReveal
                key={link.label}
                isActive={isActive}
                delayMs={220 + index * 80}
              >
                <a
                  href={link.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="post-link-card post-slide-card group flex h-full items-center justify-between gap-2"
                >
                  <div className="min-w-0 text-left">
                    <p className="post-slide-meta">{link.label}</p>
                    <p className="post-slide-card-title mt-0.5 break-words">
                      {link.value}
                    </p>
                  </div>
...
export function renderPostSlideTemplate(
  slide: PostSlide,
  isActive: boolean,
): ReactNode {
  switch (slide.kind) {
