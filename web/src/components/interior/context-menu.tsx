...
export type ContextMenuProps = {
  ...
  disabled?: boolean;
  className?: string;
  /** When true, merge trigger props onto the single child element (e.g. table row). */
  asChild?: boolean;
};
...
export function ContextMenu({
  ...
  if (asChild && isValidElement(children)) {
    ...
    trigger = cloneElement(child, {
      ...
      className: cn(
        "outline-none focus-visible:bg-primary/[0.06] focus-visible:shadow-[inset_0_0_0_1px_hsl(var(--ring))]",
        childProps.className as string | undefined,
        className,
      ),
      onContextMenu: chainHandler(
        childProps.onContextMenu as ((e: ReactMouseEvent<HTMLElement>) => void) | undefined,
        triggerProps.onContextMenu,
      ),
      onKeyDown: chainHandler(
        childProps.onKeyDown as ((e: ReactKeyboardEvent<HTMLElement>) => void) | undefined,
        triggerProps.onKeyDown,
      ),
      onPointerDown: chainHandler(
        childProps.onPointerDown as ((e: ReactPointerEvent<HTMLElement>) => void) | undefined,
        triggerProps.onPointerDown,
      ),
      onPointerMove: chainHandler(
        childProps.onPointerMove as ((e: ReactPointerEvent<HTMLElement>) => void) | undefined,
        triggerProps.onPointerMove,
      ),
      onPointerUp: chainHandler(
        childProps.onPointerUp as ((e: ReactPointerEvent<HTMLElement>) => void) | undefined,
        triggerProps.onPointerUp,
      ),
      onPointerCancel: chainHandler(
        childProps.onPointerCancel as ((e: ReactPointerEvent<HTMLElement>) => void) | undefined,
        triggerProps.onPointerCancel,
      ),
      onPointerLeave: chainHandler(
        childProps.onPointerLeave as ((e: ReactPointerEvent<HTMLElement>) => void) | undefined,
        triggerProps.onPointerLeave,
      ),
      onClick: chainHandler(
        childProps.onClick as ((e: ReactMouseEvent<HTMLElement>) => void) | undefined,
        ...
        <AnimatePresence>
          {placement ? (
            <motion.div
              ...
              className="overflow-y-auto overscroll-contain rounded-lg border border-border bg-popover p-[5px] text-popover-foreground shadow-medium outline-none"
            >
              {items.map((item, index) =>
                item.type === "separator" ? (
                  ...
                  <button
                    ...
                    {...getItemProps(index)}
                    className={cn(
                      "flex h-8 w-full cursor-default select-none items-center gap-2 rounded-md px-2.5 text-left text-[13px] outline-none",
