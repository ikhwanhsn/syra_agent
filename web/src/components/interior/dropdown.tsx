...
export function useDropdown({
  ...
  const listProps = {
    ...
    onKeyDown: (e: ReactKeyboardEvent<HTMLUListElement>) => {
  ...
  return (
    <div ref={rootRef} className={cn("relative inline-block text-left", className)}>
      <button
        {...triggerProps}
        className={cn(
          "flex h-9 select-none items-center gap-2 whitespace-nowrap rounded-md border border-border bg-popover px-3 text-[13px] font-medium text-foreground outline-none transition-[box-shadow,border-color] duration-150 disabled:opacity-50",
          open
            ? "shadow-[inset_0_1px_2px_hsl(var(--foreground)/0.08)]"
            : "shadow-soft hover:border-border focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring",
        )}
      >
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            transition={
              reduced ? NONE : { ...OPEN, opacity: { duration: 0.12, ease: EASE } }
            }
            style={{ transformOrigin: "top left" }}
            className="absolute left-0 top-[calc(100%+6px)] z-50 min-w-[224px] whitespace-nowrap rounded-md border border-border bg-popover p-[5px] text-foreground shadow-medium"
          >
            <ul
              {...listProps}
              aria-label={label}
            ...
            >
              <motion.span
                aria-hidden
                className="pointer-events-none absolute inset-x-0 top-0 h-8 rounded-md bg-muted"
                initial={false}
                animate={{
              ...
              {items.map((item, i) => {
                ...
                return (
                  <li
                    ...
                    {...getItemProps(i)}
                    className={cn(
                      "relative flex h-8 cursor-default select-none items-center rounded-md px-2.5 text-[13px]",
                      item.disabled
