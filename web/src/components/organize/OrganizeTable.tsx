...
const TYPE_FILTER_ITEMS = [
  ...
  ...ENTRY_TYPES.map((t) => ({ value: t, label: ORGANIZE_ENTRY_TYPE_LABELS[t] })),
];

const STATUS_FILTER_ITEMS = [
  { value: "all", label: "All statuses" },
  ...ENTRY_STATUSES.map((s) => ({ value: s, label: ORGANIZE_ENTRY_STATUS_LABELS[s] })),
];
...
export function OrganizeTable({
  ...
  const typeLabel =
    TYPE_FILTER_ITEMS.find((item) => item.value === typeFilter)?.label ?? "All types";
  const statusLabel =
    STATUS_FILTER_ITEMS.find((item) => item.value === statusFilter)?.label ?? "All statuses";
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <Dropdown
          ...
          value={typeFilter}
          onChange={(v) => onTypeFilterChange(v as OrganizeEntryType | "all")}
          label={typeLabel}
          placeholder="All types"
          className="w-full sm:w-[180px] [&_button]:w-full"
        />

        <Dropdown
          items={STATUS_FILTER_ITEMS}
          value={statusFilter}
          onChange={(v) => onStatusFilterChange(v as OrganizeEntryStatus | "all")}
          label={statusLabel}
