...
export interface RobinhoodLpExperimentHeroProps {
  embedded?: boolean;
  loading?: boolean;
  failed?: boolean;
  onRefresh: () => void;
...
export function RobinhoodLpExperimentHero({
  embedded = false,
  loading = false,
  failed = false,
  onRefresh,
}: RobinhoodLpExperimentHeroProps) {
  return (
    <header className={cn(overviewCardShell, "overflow-hidden rounded-3xl ring-1 ring-emerald-500/12")}>
      <div
        className="pointer-events-none absolute inset-0 opacity-[0.5]"
        style={{ background: overviewAccentBackground("invest") }}
        aria-hidden
      />
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0 space-y-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                <Droplets className="h-3.5 w-3.5 text-emerald-500" aria-hidden />
            ...
            <div className="space-y-2">
              <h1 className="text-balance text-2xl font-semibold tracking-tight text-foreground sm:text-[1.85rem]">
                LP agents on Robinhood Chain
              </h1>
              <p className="max-w-xl text-pretty text-[15px] leading-relaxed text-muted-foreground">
                Paper sim on live Uniswap pools: memecoins, USDG, and tokenized stocks. No wallet,
                no on-chain capital. Strategies compete until a leader proves positive expected value.
              </p>
            </div>
          </div>
