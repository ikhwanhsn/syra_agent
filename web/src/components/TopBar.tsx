...
import { Wallet, Menu, Coins, LogOut, LayoutGrid, Check, Copy } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { WalletState } from '@/types/api';
import { useWalletContext } from '@/contexts/WalletContext';
import { cn } from '@/lib/utils';
import { BRAND_NAME, BRAND_WORD_MARK, PLAYGROUND_TOPBAR_TOP_CLASS } from '@/lib/branding';
...
export function TopBar({
  ...
  return (
    <TooltipProvider>
      <header className={cn('min-h-14 sm:min-h-16 border-b border-border/60 bg-card/70 dark:bg-card/55 backdrop-blur-2xl backdrop-saturate-150 fixed left-0 right-0 z-[100] flex flex-col justify-center max-w-[100vw] shadow-[0_1px_0_0_hsl(0_0%_100%/0.04)_inset] dark:shadow-[0_1px_0_0_hsl(0_0%_100%/0.06)_inset]', PLAYGROUND_TOPBAR_TOP_CLASS)}>
        <div
          className={cn(
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-52">
                ...
                <DropdownMenuItem asChild>
                  <Link to="/playground/examples" className="cursor-pointer">
                    Examples
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild>
                  <Link to="/playground/explorer" className="cursor-pointer">
                    Explorer
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild>
                  <Link to="/playground/batch-test" className="cursor-pointer">
                    Batch Test
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild>
                  <Link to="/playground/format-test" className="cursor-pointer">
                    Format Test
                  </Link>
                </DropdownMenuItem>
              </DropdownMenuContent>
            ...
            <nav className="hidden lg:flex items-center gap-0.5 border-l border-border/60 pl-3 ml-1">
              {(
                [
                  ...
                  ['/playground/explorer', 'Explorer'],
                  ['/playground/batch-test', 'Batch Test'],
                  ['/playground/format-test', 'Format Test'],
                ] as const
              ).map(([to, label]) => (
                <Link
                  key={to}
          ...
          <div className="flex min-w-0 shrink-0 items-center justify-end gap-1 sm:gap-2 lg:justify-self-end">
            {wallet.connected ? (
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <p className="text-xs">Connect a Solana wallet via Privy (Phantom, Solflare, …)</p>
                </TooltipContent>
              </Tooltip>
            )}
          </div>
