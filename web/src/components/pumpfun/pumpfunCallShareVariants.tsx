...
import { PUMPFUN_CALL_SHARE_SITE_PATH } from "@/components/pumpfun/pumpfunCallShareDimensions";
import {
  deriveShareCardData,
  EMERALD,
  EMERALD_BORDER,
  ...
  function PumpfunCallShareCardIntel({ record, className }, ref) {
    ...
    return (
      <ShareCardFrame ref={ref} className={className}>
        <IntelCardBackground />
        <div className="relative z-10 flex h-full min-h-0 flex-col" style={{ padding: "28px 44px 24px" }}>
          <ShareCardHeader />

          <div
            className="relative flex min-h-0 flex-1 items-stretch"
            style={{ gap: 28, marginTop: 16, marginBottom: 10, paddingRight: 220 }}
          >
            <ShareProfilePanel
              record={record}
              scannedDate={d.scannedDate}
              statusBadge={d.statusBadge}
            />

            <div className="relative flex min-w-0 flex-1 flex-col items-center justify-center">
              <ShareHeroGain
                multiplierDisplay={d.multiplierDisplay}
                percentGain={d.percentGain}
                statusBadge={d.statusBadge}
              />
              <ShareMcapInset steps={d.mcapSteps} />
            </div>

            <SyraAgentMascot />
          </div>

          <ShareStatsRow record={record} />
          <ShareCardFooter symbol={record.symbol} verdict={record.syraAlphaVerdict || undefined} />
        </div>
      </ShareCardFrame>
    );
  },
    ...
    return (
      <ShareCardFrame ref={ref} className={className} innerRadius={8} borderColor="rgba(52,211,153,0.35)">
        ...
        >
          ...
          >
            <ShareStatsRow record={record} valueSize={22} />
          </div>

          <ShareCardFooter symbol={record.symbol} />
        </div>
      </ShareCardFrame>
    );
  },
      ...
      <ShareCardFrame ref={ref} className={className}>
        ...
        >
          <div className="flex w-full items-center justify-between">
            <p style={{ fontSize: 20, fontWeight: 800, letterSpacing: "0.2em", color: EMERALD_NEON }}>SYRA</p>
            <StatusBadge>{d.statusBadge}</StatusBadge>
          </div>
          ...
          <div className="flex flex-col items-center" style={{ gap: 24 }}>
            ...
            <div>
              ...
              <p style={{ marginTop: 6, fontSize: 17, color: EMERALD_BRIGHT }}>{record.name}</p>
            </div>
          ...
          <div style={{ width: "100%" }}>
            <ShareStatsRow record={record} valueSize={26} />
            <ShareCardFooter symbol={record.symbol} />
          </div>
        </div>
      </ShareCardFrame>
      ...
      <ShareCardFrame ref={ref} className={className} innerRadius={28}>
        <div className="relative z-10 flex h-full flex-col" style={{ padding: "44px 52px 36px" }}>
          ...
          <div style={{ marginTop: 24 }}>
            <ShareStatsRow record={record} valueSize={28} />
          </div>
          ...
          <div className="flex flex-col" style={{ width: 380, justifyContent: "space-between" }}>
            ...
            <div className="flex items-center" style={{ gap: 24 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 15, color: EMERALD_BRIGHT, marginTop: 4 }}>{record.name}</p>
