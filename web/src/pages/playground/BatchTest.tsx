...
export default function BatchTest() {
  ...
  const [paymentChain] = useState<'solana'>('solana');
  ...
  const runAll = useCallback(async () => {
    ...
    for (const entry of entries) {
      ...
      if (first.status === 402) {
        ...
        const option: X402PaymentOption | null =
          byChain.solana ?? getBestPaymentOption(parsed, 'solana');

        if (!option) {
          updateRow(entry.id, { status: 'payment_failed', statusCode: 402, error: 'No Solana payment option', timeMs: first.timeMs });
          paymentFailedCount++;
          continue;
        }

        if (isBaseNetwork(option)) {
          updateRow(entry.id, { status: 'payment_failed', statusCode: 402, error: 'Base payments not supported', timeMs: first.timeMs });
          paymentFailedCount++;
          continue;
        }
        ...
        <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 min-w-0 flex flex-col gap-8 pb-24">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-col gap-1.5 w-full sm:w-auto sm:min-w-[240px]">
              ...
              <span className="rounded-lg px-4 py-2 text-sm font-semibold bg-background/90 dark:bg-white/[0.08] text-foreground shadow-sm ring-1 ring-border/60">
                Solana
              </span>
              </div>
            </div>
