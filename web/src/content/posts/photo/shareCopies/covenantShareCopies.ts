...
export const COVENANT_PHOTO_SHARE_COPIES: Record<PostPhotoCardRole, string> = {
  cover: `Agents need an OS, not just APIs.

Syra × Covenant: open infrastructure for agent-native computing. Signed grants, audit receipts, and x402 machine money in one stack.

→ opencovenant.org`,

  thesis: `Every agent framework reinvents identity, permissions, and settlement.

Covenant provides eight host-level OS primitives via covenantd. Syra is the machine-money rail — pay-per-call intelligence over x402. Together: governed agents with verifiable receipts.`,
...
→ docs.opencovenant.org`,
  ...
  deepDive: `Covenant integration — technical surface:
