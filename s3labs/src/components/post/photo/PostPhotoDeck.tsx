...
export function PostPhotoDeck({ post }: PostPhotoDeckProps) {
  ...
  useEffect(() => {
    document.title = `S3 Labs · ${meta.title} · Photo`;
    return () => {
      ...
      <PostStudioHeader
        ...
        toolbar={
          <>
            ...
            >
              {imageCopied ? <Check className="h-4 w-4 text-cyan-400" /> : <Copy className="h-4 w-4" />}
      ...
      <div className="flex min-h-0 w-full flex-1 flex-col lg:flex-row">
        <aside className="post-studio-rail post-photo-sidebar flex flex-col">
          <div className="post-studio-rail-header">
            <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-cyan-400/75">
          ...
          <div className="border-t border-cyan-500/10 px-3 py-3 sm:px-4">
            <p className="font-mono text-[9px] uppercase tracking-[0.16em] text-cyan-400/70">
        ...
        <div className="post-studio-stage post-chrome-stage flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <p className="post-footer-hint relative z-10 mt-4 hidden text-center font-mono text-[10px] text-white/30 sm:block">
            Export each card as PNG, then paste the matching X post — 15 posts per growth brief
          </p>
        </div>
      </div>
    </PostStudioShell>
