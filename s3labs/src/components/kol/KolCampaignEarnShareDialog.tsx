...
import {
  KolCampaignEarnShareCard,
  type KolCampaignEarnShareCardData,
} from "@/components/kol/KolCampaignEarnShareCard";
import {
  buildKolCampaignEarnShareFilename,
  buildKolCampaignEarnShareTweetText,
  buildKolRankShareUrl,
  copyKolRankShareToClipboard,
  exportKolRankSharePng,
} from "@/components/kol/kolRankShareExport";

interface KolCampaignEarnShareDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  data: KolCampaignEarnShareCardData | null;
  campaignId: string;
}

export function KolCampaignEarnShareDialog({
  open,
  onOpenChange,
  data,
  campaignId,
}: KolCampaignEarnShareDialogProps) {
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      toast.success("Earnings card downloaded");
    } catch {
  ...
  const handleNativeShare = useCallback(async () => {
    ...
    try {
      await navigator.share({
        title: `${data.rewardSol.toFixed(2)} SOL KOL campaign — S3 Labs`,
        text: shareText,
        url: shareUrl,
      });
    } catch (err) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader className="space-y-1 pr-8 text-left">
          <DialogTitle className="text-xl tracking-tight">Share earnings card</DialogTitle>
          <DialogDescription>
            16:9 campaign card — flex the SOL pool and recruit KOLs on X, Telegram, and Discord.
          </DialogDescription>
        </DialogHeader>

        <div className="-mx-1 flex justify-center overflow-x-auto px-1 py-2">
          <KolCampaignEarnShareCard ref={cardRef} data={{ ...data, shareUrl }} previewScale={0.36} />
        </div>
