...
function cleanHandleInput(raw: string): string {
  return raw.trim().replace(/^@/, "").replace(/^https?:\/\/(www\.)?(x|twitter)\.com\//i, "").split(/[/?#]/)[0] ?? "";
}
              ...
              <div className="sm:text-right">
                <p
                  className={cn(
                    ...
                    row.amountKind === "paid" || row.amountKind === "held" || row.amountKind === "claimable"
                      ? "text-primary"
                      : "text-foreground",
                  )}
                >
...
function KolEarningsCheckContent() {
  ...
  return (
    <div className={cn(pageContent, "pb-20 min-w-0")}>
      ...
      <section className="mb-8 sm:mb-10 min-w-0">
        <p className="eyebrow mb-3">No wallet needed</p>
        <h1 className="heading-display">
          Check your <span className="text-gradient">SOL earnings</span>
        </h1>
        <p className="text-muted-foreground mt-4 text-base sm:text-lg leading-relaxed max-w-2xl">
          Enter an X username to see total SOL earned from KOL campaigns — paid out, held, and
          still projecting on live campaigns.
        </p>
      </section>

      <form
        onSubmit={onSubmit}
        className="flex flex-col gap-3 sm:flex-row sm:items-center max-w-xl mb-8"
      >
