...
export function PostPhotoDeck({ post }: PostPhotoDeckProps) {
  ...
  return (
    <div className="post-root post-photo-root relative flex min-h-[100dvh] w-full min-w-0 flex-col overflow-x-hidden bg-[#050807] text-white">
      <header className="post-chrome-header relative z-20 flex shrink-0 flex-wrap items-center justify-between gap-x-3 gap-y-2 px-3 py-3 sm:px-6 sm:py-4 md:px-8">
        <div className="flex min-w-0 items-center gap-2.5 sm:gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">
              Update #{meta.updateNumber} · {meta.published} · Photo
            </p>
          </div>
        </div>
        ...
        <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          ...
          >
            {imageCopied ? <Check className="h-4 w-4 text-uof" /> : <Copy className="h-4 w-4" />}
          ...
          <button
            ...
            disabled={exporting}
            className="post-play-btn inline-flex h-9 items-center gap-1.5 rounded-full border border-uof/30 bg-uof/15 px-3.5 font-mono text-[10px] uppercase tracking-[0.14em] text-uof transition-colors hover:bg-uof/25 disabled:opacity-50 sm:h-10 sm:gap-2 sm:px-5"
          >
