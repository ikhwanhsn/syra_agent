...
function AgentSettingsView({
  ...
  return (
    <div className="relative flex h-full min-h-0 min-w-0 flex-col overflow-hidden">
      ...
      >
        <ChatSidebarToggle mode="expand" onClick={onToggleSidebar} />
      </div>
      <div className="scrollbar-thin min-h-0 min-w-0 flex-1 overflow-x-hidden overflow-y-auto px-2 pb-6 pt-14 sm:px-4">
        <DashboardSettings embedded />
      </div>
    </div>
  );
}
...
export default function Index({ initialChatId, initialChat }: IndexProps = {}) {
  ...
  const handleSelectChat = useCallback(
    ...
    [location.pathname, navigate, focusChatInput],
  );
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-background overscroll-none">
      ...
      <div className="hidden min-h-0 min-w-0 flex-1 lg:flex">
        ...
        <main
          className={cn(
            "flex h-full min-w-0 flex-1 flex-col transition-[flex-grow]",
            CHAT_SIDEBAR_TRANSITION
          )}
        >
          {isSettingsRoute ? (
