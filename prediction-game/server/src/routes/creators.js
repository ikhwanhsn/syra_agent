...
router.get('/:walletAddress', async (req, res) => {
  try {
    ...
    const creator = await Creator.getOrCreate(walletAddress);
    
    // Get recent events by this creator
    const recentEvents = await Event.find({ creatorWallet: walletAddress })
      .sort({ createdAt: -1 })
      .limit(10)
...
router.get('/leaderboard/top', async (req, res) => {
  try {
    ...
    const creators = await Creator.find({ eventsCompleted: { $gt: 0 } })
      .sort({ [sortField]: -1 })
      .limit(parseInt(limit))
...
router.get('/featured/list', async (req, res) => {
  try {
    ...
    const creators = await Creator.getFeaturedCreators(parseInt(limit));
