...
// GET /api/events/stats - Get platform statistics
router.get('/stats', async (req, res) => {
  try {
    const totalEvents = await Event.countDocuments();
    const joiningEvents = await Event.countDocuments({ status: 'joining' });
    const predictingEvents = await Event.countDocuments({ status: 'predicting' });
    const waitingEvents = await Event.countDocuments({ status: 'waiting' });
    const completedEvents = await Event.countDocuments({ status: 'completed' });
    
    // Calculate totals from completed events
    const completedAggregation = await Event.aggregate([
    ...
    const allAggregation = await Event.aggregate([
      {
        $group: {
          _id: null,
          totalParticipants: { $sum: { $size: '$participants' } },
...
// POST /api/events/process-transitions - Process phase transitions
router.post('/process-transitions', async (req, res) => {
  try {
    const result = await Event.processPhaseTransitions();
    res.json({
...
router.post('/', async (req, res) => {
  try {
    ...
    const creatorProfile = await Creator.getOrCreate(creatorWallet);
...
// POST /api/events/:id/join - Join an event (pay entry fee)
...
// POST /api/events/:id/predict - Add prediction to event
...
// PUT /api/events/:id/resolve - Resolve event with final price
router.put('/:id/resolve', async (req, res) => {
  try {
    ...
    try {
      const creator = await Creator.getOrCreate(event.creatorWallet);
...
// PUT /api/events/:id/start-prediction - Manually start prediction phase
...
// GET /api/events/:id/payouts - Get detailed payout information
...
// GET /api/events/user/:walletAddress - Get events for a specific user
router.get('/user/:walletAddress', async (req, res) => {
  try {
    ...
    if (type === 'created') {
      events = await Event.find({ creatorWallet: walletAddress }).sort({ createdAt: -1 });
    } else if (type === 'joined') {
      events = await Event.find({ 'participants.walletAddress': walletAddress }).sort({ createdAt: -1 });
    } else if (type === 'predicted') {
      events = await Event.find({ 'predictions.walletAddress': walletAddress }).sort({ createdAt: -1 });
    } else {
      const created = await Event.find({ creatorWallet: walletAddress });
      const joined = await Event.find({ 'participants.walletAddress': walletAddress });
      const predicted = await Event.find({ 'predictions.walletAddress': walletAddress });
