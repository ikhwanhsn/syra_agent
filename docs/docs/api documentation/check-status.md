---
title: Check Status
sidebar_position: 13
---

# Check Status API

Health check for the API server using the x402 payment protocol.

## Overview

The Check Status API verifies that the API server is running and reachable. It is a minimal-cost paid endpoint that uses the x402 payment protocol—you must complete payment before receiving the status.

**Base URL:** `https://api.syraa.fun/v2`

**Price:** $0.0001 USD per request

## Authentication

This API uses the x402 payment protocol. On first request without payment, you'll receive a `402 Payment Required` response with payment instructions.

## Endpoints

### GET /v2/check-status

Health check.

**Example Request:**

```bash
curl https://api.syraa.fun/v2/check-status
```

**Response (Success - 200):**

```json
{
  "status": "ok",
  "message": "Check status server is running"
}
```

---

### POST /v2/check-status

Health check via POST.

**Example Request:**

```bash
curl -X POST https://api.syraa.fun/v2/check-status \
  -H "Content-Type: application/json"
```

**Response (Success - 200):** Same as GET.

## Payment Flow

### Step 1: Initial Request (402 Response)

When you first call the API without payment, you'll receive a `402 Payment Required` response:

```json
{
  "error": "Payment Required",
  "price": 0.0001,
  "currency": "USD",
  "paymentInstructions": {
    "method": "x402",
    "details": "..."
  }
}
```

### Step 2: Complete Payment

Follow the payment instructions in the 402 response.

### Step 3: Retry Request with Payment

Include the payment proof in your retry request headers to receive the status.

## Response Codes

| Code  | Description                               |
| ----- | ----------------------------------------- |
| `200` | Success - server status returned           |
| `402` | Payment Required - complete payment first  |

## Support

For payment-related issues or API support: **Telegram:** https://t.me/ikhwanhsn
