...
- Push / ship / deploy: always-on rule `.cursor/rules/push-deploy-watch.mdc` → push, wait Vercel (`syra-v2`) + Render (`syra-agent`), fix and repush until green (`node scripts/watch-deploys.mjs`)
