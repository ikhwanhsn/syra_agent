...
async function main() {
    ...
    server.tool("syra_v2_brain", "Syra Brain: single-question API. Send a natural language question; Syra selects and runs relevant tools (news, sentiment, Jupiter trending, Nansen, etc.) and returns one synthesized answer. Ideal for integrating Syra chat in one call. Swap execution not supported (use agent chat for swaps)." +
        PAYMENT_NOTE, {
    ...
    server.tool("syra_v2_squid_route", "Get cross-chain route/quote from Squid Router (100+ chains). Returns route and transactionRequest for first leg; user signs on source chain. Requires fromAddress, fromChain, fromToken, fromAmount, toChain, toToken, toAddress; optional slippage (default 1)." +
        PAYMENT_NOTE, {
    ...
    server.tool("syra_v2_squid_status", "Check status of a cross-chain transaction (Squid Router). Requires transactionId, requestId, fromChainId, toChainId; optional quoteId (required for Coral V2)." +
        PAYMENT_NOTE, {
    ...
    server.tool("syra_v2_quicknode_balance", "Quicknode: get native balance for a wallet on Solana or Base. chain (solana|base) and address required." + PAYMENT_NOTE, {
    ...
    server.tool("syra_v2_quicknode_transaction", "Quicknode: get transaction status on Solana (signature) or Base (txHash). chain and signature or txHash required." + PAYMENT_NOTE, {
        chain: z.enum(["solana", "base"]).describe("Chain: solana or base"),
    ...
    server.tool("syra_v2_quicknode_rpc", "Quicknode: forward a raw JSON-RPC request (chain: solana or base; method and params in body)." + PAYMENT_NOTE, {
        chain: z.enum(["solana", "base"]).describe("Chain: solana or base"),
        ...
        const bodyObj = { chain, method };
        if (params != null && params !== "") {
            try {
                bodyObj.params = JSON.parse(params);
            }
            catch {
                bodyObj.params = [];
            }
