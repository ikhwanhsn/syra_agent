...
async function getClient(): Promise<SyraClient> {
  if (!clientPromise) {
    clientPromise = getPaidFetch().then((fetchFn) =>
      createSyraClient({
...
function appendDevSuffix(path: string): string {
  if (!SYRA_USE_DEV_ROUTES) return path;
...
function substitutePathParams(template: string, params: Record<string, string>): string {
...
function splitParamsForTool(
  entry: McpToolCatalogEntry,
  raw: Record<string, unknown>,
): { pathParams: Record<string, string>; rest: Record<string, string> } {
...
export function formatSyraResult(success: boolean, data: unknown, error?: string, statusHint?: number): string {
  if (success) {
...
export async function callBridgeTool(
  ...
  if (!SYRA_MCP_API_KEY) {
    return {
      ok: false,
      text:
        "Agent-direct tool requires SYRA_MCP_API_KEY and server SYRA_MCP_BRIDGE_ENABLED=true. Configure MCP bridge on api.syraa.fun.",
    };
  }
...
export async function callFreeRoute(
  path: string,
