...
const LIVE_DATA_PATTERNS =
  /\b(price|prices|trading at|worth|costs?|signal|news|headline|sentiment|trending|trend|balance|balances|portfolio|swap|buy now|sell now|chart|ohlc|holders?|market cap|volume|arbitrage|smart money|whale|latest update|right now|live|today'?s|current|fetch|look up the latest|search the web|web search|scrape|crawl)\b/i;
...
const EXPLICIT_TOOL_PATTERNS =
  /\b(search the web|web search|look up the latest|find (the )?latest|get (me )?news|get (me )?a signal|show (me )?(the )?price|what(?:'s| is) .+ trading at|current price|live price)\b/i;
...
export function classifyTelegramQuestion(question) {
  ...
  if (isTelegramLiveDataQuestion(q)) return 'live_data';
