    ...
    { _id: "singleton" },
    {
      $set: {
        ...
        lastOpportunityScan: {
          ...
          diagnostics: {
            rawCandidates: scan.diagnostics?.rawCandidates ?? scan.opportunities.length,
            perSource: scan.diagnostics?.perSource ?? {},
            droppedReasons: scan.diagnostics?.droppedReasons ?? [],
            regimeReason: scan.diagnostics?.regimeReason ?? scan.regime?.reason ?? null,
            mergedCount: scan.diagnostics?.mergedCount ?? scan.opportunities.length,
            cycleSkipped: skipped.slice(0, 20),
          },
        },
      },
    },
