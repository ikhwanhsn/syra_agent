...
export function buildWelcomeEmail({ unsubscribeUrl }) {
  const subject = "You're subscribed — S3Labs KOL campaign alerts";
  const preheader =
    "We'll notify you when new KOL campaigns go live on S3Labs.";

  const bodyHtml = `
    <h1 style="margin:0 0 12px;font-size:22px;font-weight:700;color:${TEXT_COLOR};letter-spacing:-0.02em;">
      Welcome to S3Labs campaign alerts
    </h1>
    <p style="margin:0 0 20px;font-size:15px;line-height:1.65;color:${MUTED_COLOR};">
      ...
      KOL campaign goes live on the S3Labs marketplace.
    </p>
  ...
  const text = `Welcome to S3Labs campaign alerts

Thank you for subscribing. You will receive a notification each time a new KOL campaign goes live on the S3Labs marketplace.
...
export function buildUnsubscribePageHtml({ success, email }) {
  ...
  const message = success
    ? `You have been unsubscribed from S3Labs campaign alerts${email ? ` for <strong>${escapeHtml(email)}</strong>` : ""}.`
