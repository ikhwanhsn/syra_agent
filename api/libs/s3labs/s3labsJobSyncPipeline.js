/**
 * S3Labs Jobs website sync — scrape → upsert Mongo (no Telegram).
 * Keeps /jobs listings fresh regardless of Telegram post cadence.
 */

import DashboardResearch from "../../models/DashboardResearch.js";
import { S3LABS_JOB_SYNC_DB_ID } from "../../config/s3labsJobSyncConfig.js";
 ...
 * }>}
 */
export async function runS3labsJobSyncPipeline() {
  ...
  if (raw.length === 0) {
    console.warn("[s3labs-job-sync] scrape returned 0 listings — check sources");
  } else {
    console.log(
