...
describe('scheduler treasury circuit-breaker contracts', () => {
  ...
  test('random batch delay stays in 15-60 min', async () => {
    const { __test } = await import('./labX402Scheduler.js');
    for (let i = 0; i < 40; i++) {
      const d = __test.computeRandomBatchDelay();
      assert.ok(d >= __test.AUTO_BATCH_DELAY_MIN_MS, `delay ${d} < 15 min`);
      assert.ok(d <= __test.AUTO_BATCH_DELAY_MAX_MS, `delay ${d} > 60 min`);
