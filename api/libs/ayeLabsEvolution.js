/**
 * AyeLabs experiment desk — fast, smart autolearn (beyond runSimpleEvolution).
 *
 ...
 *
 * The bandit helpers (thompsonSampleStrategy / selectAyeLabsBanditLeader) are also consumed by
 * the real AyeLabs layer for explore-exploit capital allocation.
 */
...
/**
 * Thompson sampling (Beta-Bernoulli bandit) over the AyeLabs roster for explore-exploit.
 ...
 * }} [meta]
 */
export function mutateAyeLabsFromElite(parent, newId, meta = {}) {
  ...
  const parentWeights =
    p.signalWeights && typeof p.signalWeights === "object"
      ? { ...AYE_LABS_DEFAULT_SIGNAL_WEIGHTS, ...p.signalWeights }
      : { ...AYE_LABS_DEFAULT_SIGNAL_WEIGHTS };
  ...
  const signalGate =
    p.signalGate && typeof p.signalGate === "object"
      ...
      : { any: [{ field: "vl_ratio", op: "gte", value: 0.8 }], minPasses: 1 };
  ...
  return {
    strategyId: newId,
    name: `AyeLabs evo #${newId} · ${parentTag} · ${genTag}`,
    lpShape,
    binsBelow,
    binsAbove,
    screeningOverrides,
...
/**
 * Read the evolution schedule from AYE_LABS_CRON.evolution with AYE_LABS_EVOLUTION_* env overrides.
 */
export function ayeLabsEvolutionConfigFromEnv() {
  const evo = AYE_LABS_CRON.evolution;
  const num = (key, fallback) => {
    const n = Number(process.env[key]);
    return Number.isFinite(n) ? n : fallback;
  };
  const enabledRaw = String(process.env.AYE_LABS_EVOLUTION_ENABLED || "").trim().toLowerCase();
  const enabled =
    enabledRaw === "0" || enabledRaw === "false" || enabledRaw === "off"
      ...
      : enabledRaw === "1" || enabledRaw === "true" || enabledRaw === "on"
        ? true
        : evo.enabled;
  const ms = num("AYE_LABS_EVOLUTION_MS", evo.intervalMs);
  const removeCount = num("AYE_LABS_EVOLUTION_REMOVE_COUNT", evo.removeCount);
  const minDecided = num("AYE_LABS_EVOLUTION_MIN_DECIDED", evo.minDecided);
  const spawnCount = num("AYE_LABS_EVOLUTION_SPAWN_COUNT", AYE_LABS_DAILY_SPAWN_COUNT);
  const maxStrategies = num("AYE_LABS_EVOLUTION_MAX_STRATEGIES", AYE_LABS_MAX_STRATEGIES);
  return {
    enabled,
    ...
    maxStrategies: clamp(Math.floor(maxStrategies), AYE_LABS_STATIC_STRATEGY_COUNT, 99),
    pinned: new Set([AYE_LABS_REAL_MIRROR_STRATEGY_ID]),
  };
}
...
/**
 * Run one AyeLabs evolution tick: cull worst evolvable strategies, spawn guided elite mutations.
 *
 * @param {{
 *   removeCount?: number;
 *   minDecided?: number;
 ...
 * }} [opts]
 */
export async function runAyeLabsExperimentEvolution(opts = {}) {
  const cfg = ayeLabsEvolutionConfigFromEnv();
  ...
  const state = await AyeLabsState.findById("singleton").lean();
  ...
  const [ranked, statsResult] = await Promise.all([
    rankAyeLabsStrategiesByNetPnl(experimentId),
    getAyeLabsStats(),
  ]);
  ...
  const cullable = ranked
    .filter((r) => {
      ...
      if (id < AYE_LABS_EVOLVABLE_MIN_ID || id > AYE_LABS_EVOLVABLE_MAX_ID) return false;
  ...
  for (const v of cullable.slice(0, removeCount)) {
    const del = await AyeLabsStrategyOverride.deleteOne({ strategyId: Number(v.strategyId) });
    culled.push({
  ...
  const recentLessons = await AyeLabsLesson.find({ experimentId })
    .sort({ createdAt: -1 })
    .limit(3)
    .lean();
  ...
  const postCull = await resolveAyeLabsStrategies();
  const usedIds = new Set(postCull.map((s) => s.id));
  const spawned = [];
  for (let i = 0; i < spawnCount; i += 1) {
    if (elites.length === 0 || usedIds.size >= maxStrategies) break;
    let newId = null;
    for (let id = AYE_LABS_EVOLVABLE_MIN_ID; id <= AYE_LABS_EVOLVABLE_MAX_ID; id += 1) {
      if (!usedIds.has(id)) {
        newId = id;
        break;
      }
    ...
    const mutated = mutateAyeLabsFromElite(parent, newId, {
    ...
    await AyeLabsStrategyOverride.findOneAndUpdate(
      { strategyId: newId },
      { $set: { ...mutated } },
      { upsert: true },
    );
  ...
  if (spawned.length) invalidateAyeLabsStrategyCache();
