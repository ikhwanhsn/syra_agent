...
export async function runPumpfunAlphaTrendTick() {
  ...
  try {
    ...
    if (out.skipped) {
      startupVerbose("[pumpfun-alpha-trend] tick skipped — all snapshots fresh");
      return { success: true, skipped: true };
    }
    const count = out.results?.length ?? 0;
    startupVerbose(`[pumpfun-alpha-trend] OK refreshed ${count} snapshot(s)`);
...
export function startPumpfunAlphaTrendScheduler() {
  ...
  if (!Number.isFinite(ms) || ms <= 0) {
    startupVerbose("[pumpfun-alpha-trend] scheduler disabled (PUMPFUN_AGENTS_REFRESH_MS=0)");
    return;
  }
  ...
  startupVerbose(`[pumpfun-alpha-trend] scheduler started (every ${Math.round(ms / 60_000)}m)`);
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      if (!existing || isPumpfunAlphaTrendStale(existing.savedAt)) {
        startupVerbose("[pumpfun-alpha-trend] running initial or stale refresh");
