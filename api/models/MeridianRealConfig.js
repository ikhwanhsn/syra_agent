...
/**
 * Per-agent Meridian real config. _id is the Solana agent wallet pubkey (string), not ObjectId.
 * Ships disabled with hard caps — a separate live layer flips `enabled` only after the paper
 * desk graduates. Kept independent from LpRealConfig so Meridian never touches LP capital.
 */
const meridianRealConfigSchema = new mongoose.Schema(
  {
    ...
    title: { type: String, default: "Meridian Real Agent (Meteora DLMM)" },
    startedAt: { type: Date, default: Date.now },
    /** Hard cap: notional SOL per Meridian position. */
    maxPositionSol: { type: Number, default: 0.3, min: 0 },
    /** Hard cap: concurrent live Meridian positions. */
  ...
  { collection: "meridian_real_config", timestamps: true },
);

meridianRealConfigSchema.index({ enabled: 1, agentAddress: 1 });

meridianRealConfigSchema.pre("validate", function syncIdFromAddress() {
  if (this.agentAddress) {
    this._id = this.agentAddress;
  }
});

if (mongoose.models.MeridianRealConfig) {
  delete mongoose.models.MeridianRealConfig;
}

const MeridianRealConfig = mongoose.model("MeridianRealConfig", meridianRealConfigSchema);

export default MeridianRealConfig;
