...
/**
 * Live on-chain Meteora DLMM position opened by the Meridian real agent.
 * Same shape as LpRealPosition but scoped to the `meridian_real_positions` collection so
 * Meridian's live PnL and lifecycle never mix with the LP agent's positions.
 */
const meridianRealPositionSchema = new mongoose.Schema(
  {
  ...
  { timestamps: true, collection: "meridian_real_positions" },
);

meridianRealPositionSchema.index({ status: 1, openedAt: -1 });
meridianRealPositionSchema.index({ experimentId: 1, strategyId: 1, status: 1 });
meridianRealPositionSchema.index({ experimentId: 1, poolAddress: 1, status: 1, createdAt: -1 });
meridianRealPositionSchema.index({ agentAddress: 1, status: 1, openedAt: -1 });

const MeridianRealPosition =
  mongoose.models.MeridianRealPosition ||
  mongoose.model("MeridianRealPosition", meridianRealPositionSchema);

export default MeridianRealPosition;
