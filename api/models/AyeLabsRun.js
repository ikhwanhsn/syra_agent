...
/**
 * Paper (sim) DLMM position for the AyeLabs experiment desk.
 * Shape matches LpExperimentRun — SOL-based economics, Meteora pool/bin fields — but
 * lives in the `ayelabs_runs` collection so the desk keeps its own history.
 */
const ayelabsRunSchema = new mongoose.Schema(
  {
  ...
  { timestamps: true, collection: "ayelabs_runs" },
);

ayelabsRunSchema.index({ experimentId: 1, strategyId: 1, status: 1, createdAt: -1 });
ayelabsRunSchema.index({ experimentId: 1, status: 1 });
ayelabsRunSchema.index({ strategyId: 1, status: 1, createdAt: -1 });
ayelabsRunSchema.index({ status: 1, createdAt: -1 });
ayelabsRunSchema.index({ strategyId: 1, poolAddress: 1, status: 1, createdAt: -1 });
// TTL: purge settled runs after N days (default 45)
ayelabsRunSchema.index(
  { resolvedAt: 1 },
  {
...
const AyeLabsRun =
  mongoose.models.AyeLabsRun || mongoose.model("AyeLabsRun", ayelabsRunSchema);

export default AyeLabsRun;
