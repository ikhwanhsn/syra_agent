...
/**
 * Per-agent alpha sniper real config. _id is the Solana agent wallet pubkey (string), not ObjectId.
 */
const sniperRealConfigSchema = new mongoose.Schema(
  {
    ...
    title: { type: String, default: "Alpha Sniper Real Agent" },
    startedAt: { type: Date, default: Date.now },
    targetBankSol: { type: Number, default: 2, min: 0 },
    maxPositionSol: { type: Number, default: 0.4, min: 0 },
    maxConcurrentPositions: { type: Number, default: 3, min: 1, max: 20 },
    dailyLossCapSol: { type: Number, default: 0.5, min: 0 },
    reserveSolForFees: { type: Number, default: 0.05, min: 0 },
    minAlphaScore: { type: Number, default: 70, min: 0, max: 100 },
    requireRugcheckPass: { type: Boolean, default: true },
    ...
    publicMaxDepositSol: { type: Number, default: 3, min: 0.1, max: 50 },
  ...
  { collection: "sniper_real_config", timestamps: true },
);

sniperRealConfigSchema.index({ enabled: 1, agentAddress: 1 });

sniperRealConfigSchema.pre("validate", function syncIdFromAddress() {
  if (this.agentAddress) {
    this._id = this.agentAddress;
  }
});

if (mongoose.models.SniperRealConfig) {
  delete mongoose.models.SniperRealConfig;
}

const SniperRealConfig = mongoose.model("SniperRealConfig", sniperRealConfigSchema);

export default SniperRealConfig;
