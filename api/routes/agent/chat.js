...
— **Canonical assets** (BTC, majors, LSTs, RWAs, ETFs) and “should I trade this mint/ticker” research → prefer **asset-research** (resolve → Tokens risk → Syra news/sentiment/events/signal → nextActions). For raw Tokens API pieces use **tokens-*** tools. Flow: **tokens-assets-resolve** (ref or mint) then **tokens-asset-detail** / risk / OHLCV with **assetId**. Mint-only risk → **tokens-risk-summary-mint**. Batch mint snapshots → **tokens-market-snapshots** or **tokens-variant-markets**. Docs: https://docs.tokens.xyz/v1/quickstart · OSS: https://github.com/solana-foundation/tokens
...
- For **asset-research** set **params**: {"ref": "btc"} or {"mint": "<solana mint>"} or {"assetId": "solana"} when the user wants a full research decision (risk + Syra intel). Prefer this over calling resolve + risk + news/signal separately.
