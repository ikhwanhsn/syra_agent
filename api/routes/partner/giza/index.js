...
export async function createGizaRouter() {
  ...
  router.get('/protocols', async (req, res) => {
    const paid = await requirePayment(req, res, {
      ...
      inputSchema: {
        type: 'object',
        properties: { token: { type: 'string', description: 'Token contract address (e.g. USDC on Base)' } },
        required: ['token'],
      },
    });
    if (!paid) return;
  ...
  router.get('/agent', async (req, res) => {
    const paid = await requirePayment(req, res, {
      ...
      inputSchema: {
        type: 'object',
        properties: { owner: { type: 'string', description: 'Owner EOA address (0x...)' } },
        required: ['owner'],
      },
    });
    if (!paid) return;
  ...
  // POST /giza/activate — body: owner, token, protocols[], txHash, constraints?
  router.post('/activate', async (req, res) => {
    const paid = await requirePayment(req, res, {
      ...
      inputSchema: {
        ...
        required: ['owner', 'token', 'protocols', 'txHash'],
      },
    });
    if (!paid) return;
    if (!ensureGiza(res)) return;
  ...
  // POST /giza/withdraw — body: owner, amount? (optional; omit for full withdrawal)
  router.post('/withdraw', async (req, res) => {
    const paid = await requirePayment(req, res, {
      ...
      inputSchema: {
        ...
        properties: {
          owner: { type: 'string' },
          amount: { type: 'string', description: 'Optional; omit for full withdrawal' },
        },
        required: ['owner'],
      },
    });
    ...
    try {
      ...
      await maybeSettle(res, req);
      res.json({ success: true, message: amount ? 'Partial withdrawal initiated' : 'Full withdrawal (deactivation) initiated' });
    } catch (err) {
      res.status(502).json({
        success: false,
  ...
  // POST /giza/top-up — body: owner, txHash
  router.post('/top-up', async (req, res) => {
    const paid = await requirePayment(req, res, {
      ...
      inputSchema: {
        type: 'object',
        properties: { owner: { type: 'string' }, txHash: { type: 'string' } },
        required: ['owner', 'txHash'],
      },
    });
    if (!paid) return;
    if (!ensureGiza(res)) return;
  ...
  // POST /giza/update-protocols — body: owner, protocols[]
  router.post('/update-protocols', async (req, res) => {
    const paid = await requirePayment(req, res, {
      ...GIZA_PAYMENT_BASE,
    ...
    if (protocolList.length === 0) {
      res.status(400).json({ success: false, error: 'Body must include protocols (array or comma-separated string)' });
      return;
    }
    try {
      const agent = await getAgentByOwner(String(owner).trim());
  ...
  // POST /giza/run — body: owner (manual rebalance)
