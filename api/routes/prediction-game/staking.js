...
import PredictionStaking, { STAKING_TIERS } from '../../models/prediction-game/Staking.js';
...
router.get('/:walletAddress', async (req, res) => {
  try {
    ...
    const staking = await PredictionStaking.getOrCreate(walletAddress);
...
router.post('/:walletAddress/unstake', async (req, res) => {
  try {
    ...
    const staking = await PredictionStaking.findOne({ walletAddress });
...
router.get('/leaderboard/top', async (req, res) => {
  try {
    ...
    const topStakers = await PredictionStaking.find({ stakedAmount: { $gt: 0 } })
      .sort({ stakedAmount: -1 })
      .limit(limit)
...
export function createPredictionStakingRouter() {
  return router;
}
