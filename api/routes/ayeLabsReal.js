...
import {
  ...
  enableAyeLabsReal,
  getAyeLabsRealState,
  listAyeLabsRealPositions,
  resolveAyeLabsRealPositions,
  runAyeLabsRealSignalCycle,
} from "../libs/ayeLabsRealService.js";

export function createAyeLabsRealRouter() {
  const router = express.Router();

  router.get("/state", optionalWalletSession(), async (req, res) => {
    try {
      const data = await getAyeLabsRealState({
  ...
  router.get("/positions", optionalWalletSession(), async (req, res) => {
    try {
      ...
      const data = await listAyeLabsRealPositions({ limit, offset, status, agentAddress });
      res.json({ success: true, data });
    } catch (e) {
      res.status(500).json({
        success: false,
  ...
  router.post("/disable", requireSession(), async (req, res) => {
    try {
      const body = req.body && typeof req.body === "object" ? req.body : {};
      const data = await disableAyeLabsReal({
        anonymousId: req.user.anonymousId,
  ...
  router.post("/cron/signal", requireEarnExperimentCronSecret, async (_req, res) => {
    try {
      res.json({ success: true, data: await runAyeLabsRealSignalCycle() });
    } catch (e) {
      res.status(500).json({
        success: false,
        error: e instanceof Error ? e.message : String(e),
  ...
  router.post("/cron/resolve", requireEarnExperimentCronSecret, async (_req, res) => {
    try {
      res.json({ success: true, data: await resolveAyeLabsRealPositions() });
    } catch (e) {
      res.status(500).json({
        success: false,
        error: e instanceof Error ? e.message : String(e),
