...
export async function runSyraGrowthScoutAgent(input) {
  ...
  const baseMax = INTERNAL_PIPELINE_MAX_COMPLETION_TOKENS.growthScout;
  /** @type {unknown} */
  let parsed;
  /** @type {Error | null} */
  ...
  for (const maxTokens of [baseMax, Math.min(baseMax * 2, 4000)]) {
    ...
    try {
      ...
      if (!result.truncated && maxTokens === baseMax) {
        // Non-truncation parse error: still retry once with more budget (model sometimes overruns).
        continue;
      }
  ...
  if (lastParseErr || parsed == null) {
    throw lastParseErr || new Error("Growth scout failed to parse model JSON");
  }
