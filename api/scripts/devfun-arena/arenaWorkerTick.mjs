/**
 * One arena worker tick (shared by CLI `run-worker.mjs` and HTTP POST /internal/arena-worker/tick).
 */
...
async function processOneChallenge(
  ...
  await submitPumpOrDump(apiKey, challenge.id, payload);
  log("submitted", challenge.id, payload.prediction);
 ...
 *   gameType?: string;
 *   rawChallenges?: number;
 *   eligibleOpen?: number;
 *   filter?: { closed: number; alreadySubmitted: number; pastDeadline: number; insideDeadlineBuffer: number };
...
export async function runArenaWorkerTick(apiKey) {
  ...
  if (!competition) {
    ...
    return { idle: true, reason: "no_active_competition" };
  }
  ...
  if (open.length === 0) {
    ...
    return {
      idle: true,
      reason: rawList.length === 0 ? "no_open_challenges_from_api" : "all_filtered",
      competitionId: competition.id,
      competitionName: competition.name,
      rawChallenges: rawList.length,
      eligibleOpen: 0,
      filter: { closed, alreadySubmitted: already, pastDeadline: late, insideDeadlineBuffer: soon },
      dryRun: dry,
    };
  }
  ...
  let learnerUpdates = learnerW.updates;
  if (!dry && process.env.ARENA_DISABLE_SGD !== "1") {
    try {
      ...
      learnerUpdates = learnerW.updates;
  ...
  return {
    ...
    competitionName: competition.name,
    rawChallenges: rawList.length,
    eligibleOpen: open.length,
    filter: { closed, alreadySubmitted: already, pastDeadline: late, insideDeadlineBuffer: soon },
    challengesProcessed: n,
    dryRun: dry,
    learnerUpdates,
  };
}
