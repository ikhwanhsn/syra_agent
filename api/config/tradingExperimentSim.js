...
/** Legacy fallback when run rows lack notionalUsd (pre–full-capital runs). */
export const TRADING_EXPERIMENT_TRADE_NOTIONAL_USD = 200;
/** Minimum free cash required to open a new lab trade (USD). */
export const TRADING_EXPERIMENT_MIN_TRADE_NOTIONAL_USD = 1;
...
/**
 * Full-capital deployment: use all free cash (starting bank + realized P/L minus open notional).
 * @param {number} cashUsd
 * @returns {number}
 */
export function computeExperimentNotionalUsd(cashUsd) {
  const cash = Number.isFinite(cashUsd) && cashUsd > 0 ? cashUsd : 0;
  if (cash < TRADING_EXPERIMENT_MIN_TRADE_NOTIONAL_USD) return 0;
  return roundUsd(cash);
}
