...
const agentKeypair = Keypair.fromSecretKey(Uint8Array.from(JSON.parse(AGENT_KEYPAIR)));
...
export async function createAgentRouter() {
    ...
    const handleRequest = async (req, res, method) => {
        try {
            ...
            const txSignature = response.headers.get("x-solana-signature") ||
