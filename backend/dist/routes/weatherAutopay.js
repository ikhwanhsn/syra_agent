...
const connection = new Connection(SOLANA_RPC_URL || "https://api.mainnet-beta.solana.com", "confirmed");
...
export async function createWeatherAutopayRouter() {
    ...
    router.get("/", async (req, res) => {
        try {
            ...
            const initialResponse = await fetch(`${req.protocol}://${req.get("host")}/weather`, {
                method: "GET",
            });
            ...
            if (!paymentInfo.accepts || paymentInfo.accepts.length === 0) {
                throw new Error(`No payment options found in response: ${JSON.stringify(paymentInfo)}`);
            }
            ...
            const fromTokenAccount = await getAssociatedTokenAddress(usdcMint, payerKeypair.publicKey);
            ...
            const transaction = new Transaction().add(createTransferInstruction(fromTokenAccount, toTokenAccount, payerKeypair.publicKey, amountUSDC));
            ...
            const paidResponse = await fetch(`${req.protocol}://${req.get("host")}/weather`, {
                method: "GET",
                headers: {
