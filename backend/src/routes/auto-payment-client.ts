...
function parse402Response(
  ...
  if (body?.accepts && Array.isArray(body.accepts) && body.accepts.length > 0) {
    ...
    const solanaOption = body.accepts.find(
      (a: PaymentRequirement) =>
        a.network === "solana-devnet" || a.network === "solana"
    );
  ...
  if (xPayment) {
    try {
      ...
      if (decoded.accepts) {
        const solanaOption = decoded.accepts.find(
          (a: PaymentRequirement) =>
            a.network === "solana-devnet" || a.network === "solana"
        );
...
export async function createAutoPaymentRouter() {
  ...
  router.get("/", async (req, res) => {
    try {
      ...
      const paymentHeader = await createPaymentPayload(
        connection,
        wallet,
        requirement
      );
