...
const { PRIVATE_KEY_PAYAI, WEATHER_API_URL = "http://localhost:4000/weather" } =
  process.env;
...
if (!WEATHER_API_URL) {
  throw new Error(
    "WEATHER_API_URL must be set (e.g., http://localhost:3000/weather)"
  );
}
...
export async function createAutoPaymentRouter() {
  ...
  const signer = await createKeyPairSignerFromBytes(
    base58.decode(PRIVATE_KEY_PAYAI!)
  );
