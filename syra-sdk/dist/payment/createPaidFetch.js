...
import { registerBuilderCodeClientExtension, registerRequiredExtensionsHook } from "./x402Extensions.js";
...
async function createSolanaPaidFetch(fetchFn) {
    ...
    try {
        ...
        if (!isRpcBlockchainAccessError(error))
            throw error;
        switchToFallbackRpc();
        return wrapPaidFetchWithRetries(await buildPaymentFetch());
    }
...
export async function getPaidFetch(options) {
    if (!hasPaidFetchConfigured(options))
        return globalThis.fetch;
    if (!options && cachedPaidFetch)
        return cachedPaidFetch;
    const paid = await createX402PaidFetch(globalThis.fetch, options);
    if (!options)
        cachedPaidFetch = paid;
    return paid;
}
export function hasPaidFetchConfigured(options) {
    if (options?.solanaKeypair || options?.evmPrivateKey)
        return true;
    ...
    if (options?.network === "algorand") {
        return Boolean(firstNonEmptyEnv("SYRA_ALGORAND_PAYER_PRIVATE_KEY", "ALGORAND_AGENT_PRIVATE_KEY", "AVM_PRIVATE_KEY", "ALGORAND_PRIVATE_KEY"));
    }
    if (shouldUseAlgorandX402()) {
        return Boolean(firstNonEmptyEnv("SYRA_ALGORAND_PAYER_PRIVATE_KEY", "ALGORAND_AGENT_PRIVATE_KEY", "AVM_PRIVATE_KEY", "ALGORAND_PRIVATE_KEY"));
    }
...
export function getPaidFetchNetworkLabel() {
    if (shouldUseAlgorandX402())
        return "algorand";
    if (shouldUseBaseX402())
        return "base";
...
export function resetPaidFetchCache() {
    cachedPaidFetch = null;
}
export async function createPaidFetchFromKeypair(keypair, network = "solana") {
