...
function getFallbackRpcUrl(primary: string): string {
  const candidate = trimEnv("SOLANA_RPC_FALLBACK_URL");
  if (candidate && candidate !== primary) return candidate;
  if (primary === DEFAULT_PUBLIC_SOLANA_HTTP) return "https://rpc.ankr.com/solana";
...
export function getSvmRpcUrlForX402(): string {
  return useFallbackRpc ? FALLBACK_RPC : PRIMARY_RPC;
}

export function isRpcBlockchainAccessError(error: unknown): boolean {
  const parts: string[] = [];
...
export function switchToFallbackRpc(logPrefix = "[syra-sdk]"): void {
  if (!useFallbackRpc) {
    useFallbackRpc = true;
    console.error(`${logPrefix} Primary Solana RPC blocked blockchain access — switching to fallback`);
  }
}
