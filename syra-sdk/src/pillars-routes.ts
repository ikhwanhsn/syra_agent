 ...
 */
export const SYRA_PILLAR_IDS = ["earn", "treasury", "invest", "spend", "grow"] as const;
...
export function resolveSyraPillarForPath(pathname: string): SyraPillarId {
  ...
  let best: { id: SyraPillarId; len: number } | null = null;
  for (const id of SYRA_PILLAR_IDS) {
    for (const prefix of SYRA_PILLAR_ROUTES[id]) {
      ...
      if (p === pre || p.startsWith(pre.endsWith("/") ? pre : `${pre}/`) || p.startsWith(pre)) {
        if (!best || pre.length > best.len) best = { id, len: pre.length };
      }
    }
  }
