        ...
        <Card className="border-destructive/25 bg-destructive/[0.03]">
          <CardContent className="space-y-3 p-5">
            ...
            <p className="text-sm text-muted-foreground">
              {userReadableAlphaDataError((batchQ.error as Error)?.message) ||
                "Request failed. Confirm the gateway can reach /x-projects-analyze."}
            </p>
                        ...
                        <TableCell>
                          <div className="min-w-0">
                            ...
                            <p className="mt-0.5 flex items-center gap-1 text-xs text-destructive">
                              ...
                              {userReadableAlphaDataError(item.error)}
                            </p>
                          </div>
                        </TableCell>
...
function RiseAlphaTabPanel() {
  const [period, setPeriod] = useState<PumpfunAlphaPeriod>("today");

  const batchQ = useQuery({
    queryKey: ["alpha", "rise-intel", period],
  ...
  return (
    <div className="space-y-8 pb-10 sm:pb-12 lg:pb-14">
        ...
        <CardContent className="relative space-y-5 p-5 sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="min-w-0 space-y-2">
              ...
              <h2 className="text-xl font-semibold tracking-tight text-foreground sm:text-2xl">Borrow-aware Alpha sleeve</h2>
              <p className="max-w-2xl text-sm leading-relaxed text-muted-foreground/90">
                Fund-style telemetry layered on the same graduate tape as Pumpfun — utilization, borrow APR, and sleeve NAV
                are modeled paper lenses for product UI (not on-chain execution). Pair it with the Rise experiment to stress
                two sniper desks that can tap the vault when cash runs thin.
              </p>
      ...
      {batchQ.isLoading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          <KpiTile
            label="Borrow pool (modeled)"
            value={formatCompactUsd(rise.borrowPoolUsd)}
            hint="Illustrative vault depth sized off the live tape leader."
            className="ring-1 ring-sky-500/10"
          />
          <KpiTile label="Pool utilization" value={`${rise.utilizationPct.toFixed(1)}%`} hint="Borrowed vs available liquidity." />
          <KpiTile label="Borrow APR" value={`${rise.borrowAprPct.toFixed(2)}%`} hint="Variable borrow for levered clips." />
          <KpiTile label="Lend APR" value={`${rise.lendAprPct.toFixed(2)}%`} hint="Supply-side yield mirror." />
          <KpiTile label="Alpha sleeve NAV" value={formatCompactUsd(rise.alphaNlvUsd)} hint="Net liquid value for the Rise Alpha sleeve." />
          <KpiTile label="24h sleeve flow" value={formatSignedCompactUsd(rise.flow24hUsd)} hint="Modeled net deposits minus withdrawals." />
        </div>
      ) : null}
      ...
      {batchQ.isError ? (
        <Card className="border-destructive/25 bg-destructive/[0.03]">
          <CardContent className="space-y-3 p-5">
            <p className="text-sm font-medium text-foreground">Unable to load Rise Alpha lens</p>
            <p className="text-sm text-muted-foreground">
              {(batchQ.error as Error)?.message || "Request failed. Confirm the gateway can reach the market endpoints."}
            </p>
            <Button
        ...
        batchQ.data.tokens.length === 0 ? (
          ...
          <>
            <div className="grid gap-6 xl:grid-cols-3">
              ...
              <Card className="border-border/55 bg-card/60 backdrop-blur-md">
                <CardHeader className="pb-3">
                  <CardTitle className="text-lg">Rise Alpha mints</CardTitle>
                  <CardDescription>Watchlist + tape leader — targets for the Rise-only sniper in the experiment.</CardDescription>
                </CardHeader>
                <CardContent>
                  {batchQ.data.riseAlphaMintTargets.length ? (
                    <div className="space-y-3">
                      {batchQ.data.riseAlphaMintTargets.map((mint) => {
                        const sym =
                          batchQ.data.analysis.watchlist.find((w) => w.mint === mint)?.symbol ??
                          batchQ.data.tokens.find((t) => t.mint === mint)?.symbol ??
                          "RISE";
                        return (
                          <div key={mint} className="rounded-xl border border-border/45 bg-muted/[0.08] p-4">
                            <div className="flex items-center justify-between gap-2">
                              <div className="min-w-0">
                                <p className="truncate font-mono text-sm font-semibold text-foreground">{sym}</p>
                                <p className="mt-0.5 truncate font-mono text-xs text-muted-foreground">{mint}</p>
                              </div>
                              <a
                    ...
                    <p className="text-sm text-muted-foreground">No Rise Alpha mint targets yet.</p>
                  )}
                </CardContent>
              </Card>
            </div>
            ...
            <Card className="border-border/55 bg-card/55 backdrop-blur-md">
              <CardHeader className="pb-3">
                <CardTitle className="text-lg">Matched tokens</CardTitle>
                <CardDescription>Complete pump.fun graduates — shared tape for Rise + Pumpfun tabs.</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                  <Table>
                          ...
                          <TableCell>
                            <div className="min-w-0">
                              <div className="flex items-center gap-2">
                                <p className="truncate font-semibold tracking-tight text-foreground">{t.symbol}</p>
                                {batchQ.data.riseAlphaMintTargets.includes(t.mint) ? (
                                  <Badge variant="outline" className="h-5 shrink-0 border-sky-500/35 bg-sky-500/[0.08] px-1.5 text-[10px] font-semibold uppercase tracking-wide text-sky-100">
                                    Rise
                                  </Badge>
                                ) : null}
                              </div>
