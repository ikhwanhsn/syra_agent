 ...
 */
export async function fetchWithX402Payment(url, init, options) {
    const header = options.paymentHeader ?? "PAYMENT-SIGNATURE";
    const maxRetries = options.maxRetries ?? 2;
    let retries = 0;
    const attempt = async (withPayment) => {
        const headers = new Headers(init.headers);
        if (withPayment)
            headers.set(header, withPayment);
        ...
        if (!res.ok) {
            ...
            return {
                ...
                charged: !safe && res.ok,
                retries,
                error: typeof json === "object" && json && "error" in json ? String(json.error) : text.slice(0, 200),
            };
        }
        return {
            ok: true,
